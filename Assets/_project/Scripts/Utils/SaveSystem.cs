...
{
    public static void SaveData(string dataName, int levelNo, float healthRatio, Vector3 playerPosition)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/" + dataName + ".fun";
        FileStream stream = new FileStream(path, FileMode.Create);        

        StoreData data = new StoreData(levelNo, healthRatio, playerPosition);
